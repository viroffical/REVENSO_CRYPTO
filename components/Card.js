...
const Card = ({ profile, dragProgress }) => {
  return (
    <div className="relative w-full h-full bg-white rounded-2xl overflow-y-auto overscroll-none">
      <div className="relative h-[50vh] sm:h-[60vh] md:h-[65vh]">
        {/* Tag on top (e.g., "bizz") */}
        {profile.badge && (
          <div className="absolute top-4 left-4 z-10">
            <div className="bg-gradient-to-r from-orange-400 to-orange-500 text-white px-4 py-1 rounded-full font-medium text-sm">
              {profile.badge}
            </div>
          </div>
        )}
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/90 via-black/60 to-transparent text-white">
          <div className="flex items-center mb-1">
            <h2 className="text-xl font-bold mr-2">{profile.name}</h2>
            {profile.verified && (
              <FontAwesomeIcon icon={faCheckCircle} className="h-5 w-5 text-blue-400" />
            )}
          </div>
          ...
          {/* Bio */}
          <p className="text-sm text-gray-100 mb-1">{profile.bio}</p>
          ...
          <div>
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                ...
                <span className="font-medium">Bumble Meetup</span>
              </div>
